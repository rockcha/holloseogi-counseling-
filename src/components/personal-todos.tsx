    ...
    return (
      <div className="space-y-5" aria-busy={loading || busy}>
        {!loading && !loadError && (
          <>
            <p className="sr-only" role="status">
              남은 할 일 {remaining}개 · 완료 {items.length - remaining}개
            </p>
          </>
        )}
        <form
        ...
        >
          ...
          <Button
            type="submit"
            disabled={!draft.trim() || loading || loadError || busy}
          ...
          <div className="flex w-fit gap-1 rounded-lg bg-[#edf1f6] p-1">
            {(
              ...
              <button
                ...
                className={`rounded-md px-3 py-1.5 text-xs transition-colors ${todoFilter === value ? "bg-white text-primary shadow-sm" : "text-[#62738a] hover:text-[#426083]"}`}
              >
                {label}
              </button>
            ))}
        ...
        {loading ? (
          ...
          <div role="alert" className="py-6 text-center space-y-3">
            <p className="text-sm">할 일을 불러오지 못했습니다.</p>
            <Button
              variant="outline"
          ...
          <>
            {items.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-[#e1e7ef] px-5 py-12 text-center">
                ...
                <p className="mt-4 text-sm font-semibold text-[#426083]">
                  작은 일부터 하나씩
                </p>
                <p className="mt-2 text-xs text-muted-foreground">
                  챙겨야 할 일을 적고, 끝내면 체크해 보세요.
                </p>
              </div>
            ) : (
                  ...
                  {visibleItems.length ? (
                    <ul className="space-y-2">
                      {visibleItems.map((item) => (
                        <li
                          key={item.id}
                          className={`group flex items-center gap-3 rounded-md border px-4 py-3 transition-colors ${item.completed ? "border-transparent bg-[#f7f8fa]" : "border-[#e5eaf1] bg-white hover:border-[#c7d4e3] hover:bg-[#fcfdff]"}`}
                        >
                          <label className="flex flex-1 min-w-0 cursor-pointer items-start gap-3">
                            ...
                            />
                            <span
                              className={`text-sm leading-6 break-words min-w-0 ${item.completed ? "line-through text-muted-foreground" : ""}`}
                            >
                              {item.title}
                            </span>
                          </label>
