...
export function FloatingMemo({ container = null, onDirtyChange }: { container?: HTMLDivElement | null; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  function clamp(p: Position): Position {
    ...
    const height = panel.current?.offsetHeight ?? Math.min(600, window.innerHeight - 96)
    return { x: Math.max(12, Math.min(p.x, window.innerWidth - width - 12)), y: Math.max(12, Math.min(p.y, window.innerHeight - height - 12)) }
  }
  function remember(p: Position) { try { localStorage.setItem(positionKey, JSON.stringify(p)) } catch { /* Position is optional. */ } }
  useEffect(() => {
    try { const p = JSON.parse(localStorage.getItem(positionKey) ?? 'null'); if (p && Number.isFinite(p.x) && Number.isFinite(p.y)) setPosition(clamp(p)) } catch { /* Default bottom-left position. */ }
  }, [positionKey])
  useEffect(() => {
    let active = true
    ...
    async function load() {
      try {
        ...
        if(supabase) {
          ...
          const {data,error}=await supabase.from('personal_memos').select('content').eq('owner_id',member.id).maybeSingle()
          if(error) throw error
          text=data?.content ?? ''
        } else text=localStorage.getItem('holoseogi-demo-memo') ?? ''
        if(active){setContent(text);setSaved(text);setMessage('')}
      } catch { if(active){setFailed(true);setMessage('메모를 불러오지 못했습니다. 연결 및 테이블 설정을 확인해 주세요.')} }
  ...
  async function save() {
    ...
    try {
      if(supabase){
        if(!member)throw new Error('Missing member')
        const {error}=await supabase.from('personal_memos').upsert({owner_id:member.id,content:snapshot},{onConflict:'owner_id'})
        if(error)throw error
      }else localStorage.setItem('holoseogi-demo-memo',snapshot)
      setSaved(snapshot);setMessage('')
    }catch{setMessage('자동 저장하지 못했습니다. 작성 내용을 유지하고 다시 시도합니다.')}
