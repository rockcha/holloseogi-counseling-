...
test("오늘 상담 학생과 메모장 배치 및 정렬 상태", async ({ page }) => {
  ...
  const memo = page.getByRole("region", { name: "메모장", exact: true });
  await expect(
    page.getByRole("region", { name: "상담 예정", exact: true }),
  ).toHaveCount(0);
  await expect(
    done.getByRole("button", { name: /상담 상세 보기/ }).first(),
  ).toHaveAttribute("aria-label", "W03 c 상담 상세 보기");
  await done.getByRole("button", { name: "오래된 순" }).click();
  await expect(
    done.getByRole("button", { name: /상담 상세 보기/ }).first(),
  ).toHaveAttribute("aria-label", "W04 d 상담 상세 보기");
  await expect(done.getByRole("button", { name: "오래된 순" })).toHaveAttribute(
    "aria-pressed",
    "true",
  );
  await expect(memo.getByRole("textbox", { name: "메모 내용" })).toBeVisible();
});

test("새 전달과 내 글 댓글만 알림, 상대 시간, 읽음 및 본문 영역", async ({
  page,
}) => {
  await page.addInitScript(() => {
    ...
    localStorage.setItem(
      "holoseogi-announcement-comments",
      JSON.stringify([
        {
          ...comment,
          id: "c1",
        ...
        },
        {
          ...
          id: "c2",
          announcement_id: "own",
          author_id: "local-teacher",
        },
        {
          ...comment,
          id: "c3",
          announcement_id: "other",
          author_id: "third-teacher",
        },
        {
          ...comment,
          id: "c4",
          announcement_id: "suggestion",
          author_id: "third-teacher",
        },
      ]),
    );
  ...
  await expect(panel.locator(".notification-row").first()).toContainText(
    "5분 전",
  );
  await panel.getByRole("button").filter({ hasText: "내 글에 댓글" }).click();
  await expect(
    page.getByRole("heading", { name: "내 글", exact: true }),
  ).toBeVisible();
  await expect(
    page.getByRole("region", { name: "본문", exact: true }),
  ).toContainText("짧은 본문");
  const height = await page
    .getByRole("region", { name: "본문", exact: true })
