...
test('회원가입 비밀번호 확인, 실명 전송 및 즉시 관리자 승인 대기',async({page})=>{
  let payload: Record<string,unknown> | null=null
  await page.route('https://counsel-test.supabase.co/auth/v1/signup',async route=>{
    payload=route.request().postDataJSON()
    await route.fulfill({json:session})
  })
  await page.route('https://counsel-test.supabase.co/rest/v1/profiles*',route=>route.fulfill({json:{id,name:'김선생',is_teacher:false}}))
  await page.goto('/')
